...
route here is mounted under /api/review.
    ...
    GET    /api/review/preferences          current default mode
    POST   /api/review/preferences          change it
    GET    /api/review/stats                counts for the panel

Validation lives here rather than in database.py because it is a rule about
what a person is allowed to submit, not about storage. Numbers are clamped
rather than rejected, so a slider that overshoots does not lose the reviewer's
work; anything unrecognised is stored untouched.
"""
...
# Fields the editor is allowed to write. Everything else on the analysis is
# carried over from the model's own output when the correction is saved.
EDITABLE = {
    "primary_emotion", "secondary_emotions", "mixed_emotion",
...
def clean(submitted, original):
    ...
    for key in ("primary_emotion", "rasa", "parjaay", "tradition", "language"):
        value = out.get(key)
        out[key] = value.strip() or None if isinstance(value, str) else value

    # Provenance travels with the verdict so nothing downstream mistakes a
    # corrected reading for a model one.
    out["edited"] = True
    out.pop("status", None)
    return out
...
def read_corrections():
    ...
    return jsonify({
        "corrections": Database.list_corrections(limit, include_retired),
